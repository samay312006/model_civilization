...
- Produces (contract-exact — this file IS the contract's Core Types block, reproduced verbatim with the three helpers implemented):
  ...
  - **Known scoped-down deviation from the spec (documented here, not fixed — this is the contract-exact type and Section 1 does not alter frozen contract types):** the spec (`docs/superpowers/specs/2026-07-04-genesis-civ-sim-design.md`, section 4) targets a population of 1,000-3,000 agents at comfortable speed via a struct-of-arrays/typed-array engine layout. The frozen contract types `SimConfig.startPopulation` (`200 | 400 | 600`, this file) and `EngineCtx.people: Person[]` (Section 2) commit instead to an array-of-full-objects hot-path layout with a hard ceiling of 600 starting agents. This is a deliberate scope reduction, not an oversight: a plain `Person[]` is far simpler to implement correctly across Sections 2-7 (agents, brains, society, sim) than a SoA/typed-array layout, and 600 agents is the population this plan's tasks are written, tested, and perf-checked against (see Task 45's smoke tests). Reaching the spec's 1,000-3,000 target would require a SoA/typed-array rewrite of the engine's internal `Person` representation, which is out of scope for this plan and would need its own contract revision. No task in this plan raises `startPopulation` above 600 or benchmarks tick performance above that ceiling.
